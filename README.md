...
[![Ant Design](https://img.shields.io/badge/Ant_Design-1677FF?style=flat&logo=antd&logoColor=white)](https://ant.design/)
[![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat&logo=fastapi&logoColor=white)](https://fastapi.tiangolo.com/)
[![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat&logo=postgresql&logoColor=white)](https://www.postgresql.org/)
[![REST API](https://img.shields.io/badge/API-REST-6B7280?style=flat)](./)
...
Components and data flow at a high level (browser, API, persistence).
...
flowchart LR
  ...
  subgraph server [Backend]
    API[FastAPI\nREST + JWT auth]
    SVC[Domain services\nbookings slots notifications calendar]
  end
...
Typical lifecycle for an office-hour booking (professor-controlled approval).

```mermaid
stateDiagram-v2
  [*] --> pending: Student requests seat
  pending --> approved: Professor approves
  pending --> rejected: Professor rejects
  pending --> cancelled: Student or policy cancels
  approved --> cancelled: Cancel allowed by rules
  rejected --> [*]
  cancelled --> [*]
  approved --> [*]: Completed / past slot
```

*Legacy stored statuses `booked` and `queued` are treated as aligned with **`approved`** and **`pending`** in reporting and dashboards.*

### User roles and main capabilities

Which actors touch which parts of the system.

```mermaid
flowchart TB
  subgraph students [Students]
    A[Browse professor slots]
    B[Booking + descriptions + chat]
    C[Classroom reservations]
    D[Calendar + notifications]
  end

  subgraph professors [Professors]
    E[Create / edit slots + capacity]
    F[Approve or reject bookings]
    G[View calendar and exports]
  end

  subgraph admins [Administrators]
    H[University-wide oversight]
    I[Analytics and classroom data]
    J[Calendar across all professors]
  end

  students --> CORE[(Shared platform)]
  professors --> CORE
  admins --> CORE
```
...
How **office-hour bookings** relate to **slots** separately from **classroom reservations**.

```mermaid
flowchart LR
  subgraph office [Office hours]
    SL[Slot]
    BK[Booking]
    SL --> BK
  end
